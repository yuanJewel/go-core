...
func (Object) AuthenticateApi(ctx iris.Context) {
	...
	if err := json.Unmarshal(returnBody, &returnObject); err != nil {
		ApiReturnErr(JsonUnmarshalError, ctx, err, response)
		return
	}
	ctx.StatusCode(code)
...
func irisAuthenticate(check func(iris.Context) error) iris.Handler {
	return func(ctx *context.Context) {
		...
		user, _, _, err := ParseToken(ctx)
		if err != nil {
			ApiReturnErr(ParseTokenEorror, ctx, err, response)
			return
		}
		if user == "admin" {
			ctx.Next()
		...
		if err := check(ctx); err != nil {
			ApiReturnErr(PermissionDeny, ctx, err, response)
			return
		} else {
			ctx.Next()
		}
