...
// newPop3Client returns a new client object using an existing connection.
func newPop3Client(opt Opt) *Client {
...
func (c *Client) NewConn() (*Conn, error) {
	...
	err = pCon.Auth(c.opt.User, c.opt.Password)
	if err != nil {
		return nil, err
	}
...
// convertGB2312ToUTF8 converts GB2312 encoded text to UTF-8
func convertGB2312ToUTF8(input string) (string, error) {
	// Create a transformer to convert GB2312 to UTF-8
	reader := transform.NewReader(strings.NewReader(input), simplifiedchinese.GB18030.NewDecoder())
	decodedBytes, err := io.ReadAll(reader) // Read all decoded bytes
	if err != nil {
		return "", err
	}
...
// parseSubject parses a subject with encoded words in the format =?charset?encoding?encoded_string?=
func parseSubject(subject string) (string, error) {
	// Regex to match encoded words, e.g., =?charset?encoding?encoded_string?=
	re := regexp.MustCompile(`=\?([a-zA-Z0-9\-]+)\?([BQbq])\?([^\?]+)\?=`)
	...
	for _, match := range matches {
		...
		if encoding == "B" || encoding == "b" {
			// Base64 decode
			decodedText, err = decodeBase64(encodedText)
		} else {
		...
		// Convert to UTF-8 if charset is GB2312
		if charset == "GB2312" {
			decodedText, err = convertGB2312ToUTF8(decodedText)
			if err != nil {
				return "", err
			}
		}

		// Add decoded text to the result
		result = append(result, decodedText)
	}
